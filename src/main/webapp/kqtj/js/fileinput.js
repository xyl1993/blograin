...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        init: function (options) {
            ...
            self.browseLabel = options.label == null ? '上传图片': options.label;
            self.isHeader = options.isHeaderImg == null ? false : options.isHeaderImg;
            self.upType = options.upType == null ? false : options.upType;
            self.reader = null;
            self.formdata = {};
            self.isIE9 = isIE(9);
            self.isIE10 = isIE(10);
            ...
            self.setEllipsis();
            self.deviceSystem = options.deviceSystem;
        },
        ...
        initFileActions: function () {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                $el.off('click').on('click', function () {//删除图片的处理
                	self.removeFile($frame,ind);
                });
            });
        ...
        removeFile : function($frame,ind) {
        	var n,cap,self = this;
        	$frame.fadeOut('slow', function () {
        		self.filestack[ind] = undefined;
        		self.uploadSucFile[ind] = undefined;
        		self.clearObjects($frame);
        		$frame.remove();
        		var filestack = self.getUploadSucFile(), len = filestack.length,
        ...
        removeFilecw : function(begin,end) {
        	var self = this,previewId,$thumb;
        	for(var i=begin;i<=end;i++) {
        		previewId = self.previewInitId + "-" + i;
				$thumb = $('#' + previewId + ':not(.file-preview-initial)');
        		self.filestack[i] = undefined;
        		self.uploadSucFile[i] = undefined;
        		self.clearObjects($thumb);
        		$thumb.remove();
        	}
        },
        /**
         * 删除单个图片
         */
        removeFileSign : function($thumb,ind) {
        	var self=this;
        	self.filestack[ind] = undefined;
    		self.uploadSucFile[ind] = undefined;
    		self.clearObjects($thumb);
        ...
        ajaxSubmit: function (fnBefore, fnSuccess, fnComplete, fnError) {
            ...
            self.uploadExtra(self.uploadUrl);
            settings = $.extend({
                xhr: function () {
        ...
        uploadBatch: function () {
            ...
            fnBefore = function (jqXHR) {
            	jqXHR.setRequestHeader("token", self.token);
            	jqXHR.setRequestHeader("deviceSystem", self.deviceSystem);
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
            	var outData = self.getOutData(jqXHR, data),previewId,$thumb;
            	//根据云平台需求修改图片上传结束后的处理     modify by wangxingchen 2015-04-01
            	//云平台图片上传成功后返回的是一个list，这里需要循环处理 每张图片的上传结果
            	var errMsg = "";
            	if(data[0]){
            		//上传多个图片
	            		if(data[0].data == undefined && data[0].code != 0) {//图片上传失败
		            		if(data[0].code=='8024'){
		            			data[0].errorMessage = "上传图片个数不能超过9个";
		            			errMsg += data[0].errorMessage +'\n';
		            		}
		            		//所需展示失败信息，将上传失败的图片删去
		        			$.each(self.filestack, function(i,d) {
		        				if(d !== undefined) {
		        					previewId = self.previewInitId + "-" + i,
		        					$thumb = $('#' + previewId + ':not(.file-preview-initial)');
		        					self.removeFileSign($thumb,i);//删除上传失败的图片
		        				}
		        			});
		            		...
		            		$.each(data,function(i,d) {
		                		var fileIdxNum = 0;
		                		//循环本次上传的图片，得到图片的位置
		                		$.each(outData.files,function(j,fd) {
		                			if(fd !== undefined && fd.name==d.data.picName) {//TODO图片上传结果需要返回图片原名称
		                				fileIdxNum = j;
		                			}
		                		});
		                		previewId = self.previewInitId + "-" + fileIdxNum;
		            			$thumb = $('#' + previewId + ':not(.file-preview-initial)');
		            			self.filestack[fileIdxNum] = undefined;
		                		if(d.code == 0) {//图片上传成功
		                			$.each(d.data, function(key,val) {//将图片保存到数组，便于获取
		                				if(key.indexOf("fileUrl") == 0) {
		                					self.uploadSucFile[fileIdxNum]= val;
		                				}
		                			});
		                			
		                			setIndicator(fileIdxNum, 'indicatorSuccess', 'indicatorSuccessTitle');
		                			self.raise('fileuploaded', [outData, previewId, fileIdxNum]);
		                			self.initUploadSuccess(data,$thumb,false);
		                		}else if(d.code == 8023){//图片上传失败,图片格式不正确
		                			d.message = window.localStorage.lang==='en'?"Image format is not correct, only support jpg,png,gif,jpeg":"图片格式不正确，只支持jpg,png,gif,jpeg";
		                			//所需展示失败信息，将上传失败的图片删去
		                			errMsg += d.message +'\n';
		                			self.removeFileSign($thumb,fileIdxNum);//删除上传失败的图片
		                			self.showUploadError(errMsg, outData, 'filebatchuploaderror');
		                		}else if(d.code == 8022){//图片上传失败,图片大小超过5M
		                			d.message = window.localStorage.lang==='en'?"Picture size can not exceed 5M":"图片大小不能超过5M";
		                			//所需展示失败信息，将上传失败的图片删去
		                			errMsg += d.message +'\n';
		                			self.removeFileSign($thumb,fileIdxNum);//删除上传失败的图片
		                			self.showUploadError(errMsg, outData, 'filebatchuploaderror');
            		...
            		var i = self.uploadSucFile.length-1;
            		previewId = self.previewInitId + "-" + i,
            		$thumb = $('#' + previewId + ':not(.file-preview-initial)');
            		if(data.data == undefined && data.code != 0) {//图片上传失败
	            		if(data.code=='8024'){
	            			data.errorMessage = window.localStorage.lang==='en'?"Upload pictures can not exceed 9":"上传图片个数不能超过9个";
	            			errMsg += data.errorMessage +'\n';
	            		}else if(data.code == 8023){//图片上传失败,图片格式不正确
	            			data.message = window.localStorage.lang==='en'?"Image format is not correct, only support jpg,png,gif,jpeg":"图片格式不正确，只支持jpg,png,gif,jpeg";
                			//所需展示失败信息，将上传失败的图片删去
                			errMsg += data.message +'\n';
                			//用户头像
	            			if(self.isHeader){
	            				$(".file-preview-thumbnails").trigger('fileError',[errMsg]);//触发事件
	    					}
	            			//会话发送图片
	            			if(self.upType){
	            				$(".file-preview-thumbnails").trigger('sendError',[errMsg]);//触发事件
	            			}
                		}else if(data.code == 8022){//图片上传失败,图片大小超过5M
                			data.message = window.localStorage.lang==='en'?"Picture size can not exceed 5M":"图片大小不能超过5M";
                			//所需展示失败信息，将上传失败的图片删去
                			errMsg += data.message +'\n';
                			//用户头像
	            			if(self.isHeader){
	            		...
	            		//所需展示失败信息，将上传失败的图片删去
	            		self.removeFileSign($thumb,self.uploadSucFile.length-1);//删除上传失败的图片
	        			self.showUploadError(errMsg, outData, 'filebatchuploaderror');
	            	}else {
	            		//上传单个图片
	            		if(data.success){
	    					self.uploadSucFile[i] = data.fileUrlWeb;
	    					setIndicator(0, 'indicatorSuccess', 'indicatorSuccessTitle');
                			self.raise('fileuploaded', [outData, previewId, 0]);
                			self.initUploadSuccess(data,$thumb,false);
	            			//用户头像
	            			if(self.isHeader){
	            				$(".file-preview-thumbnails").trigger('fileChange');//触发事件
	    					}
	            			//会话发送图片
	            			if(self.upType){
	            				$(".file-preview-thumbnails").trigger('sendOk');//触发事件
	            			}
	            		}
            	 }
            	}
...
//            	}
        ...
        getUploadSucFile: function () {
            var self = this;
            return self.uploadSucFile.filter(function (n) {
                return n !== undefined;
            });
        },
            ...
            function readFile(i) {
                ...
                if (fileCount === 0 && !isEmpty(fileExt) && isArray(fileExt) && !isEmpty(fileExtExpr)) {
                    ...
                    if (fileCount === 0) {
                        ...
                        self.isError = throwError(msg, file, previewId, i);
                        //将错误的文件删除
    					self.removeFilecw(ctr,node);//删除上传失败的图片
                        return;
                    }
                }
                if (!self.showPreview) {
                    self.filestack.push(file);
                    self.uploadSucFile.push(undefined);
        ...
        change: function (e) {
            ...
            total = self.isUploadable ? self.getUploadSucFile().length + tfiles.length : tfiles.length;
            if (self.maxFileCount > 0 && total > self.maxFileCount) {
            	if(self.isHeader) {
            		var i = self.uploadSucFile.length-1;
            		var previewId = self.previewInitId + "-" + i,
            		$thumb = $('#' + previewId + ':not(.file-preview-initial)');
            		self.removeFileSign($thumb,i);//删除上传失败的图片
            	}else {
    ...
    $.fn.fileinput.locales.en = {
        ...
        removeTitle: '清空选择的文件',
        cancelLabel: '取消',
        cancelTitle: '终止正在上传的文件',
        uploadLabel: '上传',
        uploadTitle: '上传选择的文件',
        msgSizeTooLarge: '文件 "{name}" (<b>{size} KB</b>) 超过最大允许上传大小 <b>{maxSize} KB</b>. 请重新上传文件!',
        msgFilesTooLess: '必须选择至少一个 <b>{n}</b> {files} 上传. 请重新上传文件!',
        msgFilesTooMany: '选择上传的文件个数为 <b>({n})</b> 超过最大允许的个数 <b>{m}</b>. 请重新上传文件!',
        msgFileNotFound: '文件 "{name}"没有找到!',
        msgFileSecured: '安全限制防止读取文件 "{name}".',
        msgFileNotReadable: '文件 "{name}" 不可读.',
        msgFilePreviewAborted: '文件 "{name}"预览失败.',
        msgFilePreviewError: '读取文件 "{name}"发生一个错误.',
        msgInvalidFileType: '无效的文件 "{name}". 只支持"{types}" 格式.',
        msgInvalidFileExtension: '无效的文件"{name}". 只支持 "{extensions}" 格式.',
        msgValidationError: '文件上传失败',
        msgLoading: '上传第 {index} 个 {files}文件 &hellip;',
        msgProgress: '上传第 {index} 个 {files} 文件- {name} - 完成{percent}% .',
        msgSelected: '{n} 文件已选择',
    ...
    $.fn.upload = function (ek) {
   	 var $this = $(this),data = $this.data('fileinput');
   	if (!data || !data.isUploadable) {
   		return;
   	}
   	ek.preventDefault();
   	if (!$(this).hasClass('disabled') && isEmpty($(this).attr('disabled'))) {
